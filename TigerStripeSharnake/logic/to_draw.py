...
def ev(board: np.ndarray, player: Position, enemy: Position, xturns: int, length: int):
    ...
    sssss = np.zeros(bd.shape, dtype=np.float16)
    score0, score1 = 0, 0
    x, y = enemy
    vis[x][y] = 1
    queue = [(x, y, 1)]
    ...
    if XXX:
        from ..utils.draw import draw_matrix_image
        print(sssss)
        draw_matrix_image(sssss, 80, 'pic.jpg')
        XXX = False
...
def ev(board: np.ndarray, player: Position, enemy: Position, xturns: int, length: int):
    bd = (xturns - abs(board) >= length)
    vis = np.zeros(bd.shape, dtype=np.int8)
    ssssse = np.zeros(bd.shape, dtype=np.float16)
    sssss = np.zeros(bd.shape, dtype=np.float16)
    score0, score1 = 0, 0
    x, y = enemy
    vis[x][y] = 1
    ...
    if XXX:
        XXX = False
        from ..utils.draw import draw_matrix_image
        draw_matrix_image(sssss, 100, 'mpic.jpg')
        draw_matrix_image(-ssssse, 100, 'epic.jpg')
        draw_matrix_image((sssss-ssssse), 100, 'vspic.jpg')
        ...
        if INFO:
            ...
            while lst != []:
                ...
                for each,fa in ls:
                    cntz += 1
                    if ceng <= PRINT_DEPTH:
                        dot.node(str(cntz), f'vis:{each.vis}\nrew:{round(each.reward / each.vis, 2)}\nrewE:{round(each.rewardE / each.vis, 2)}')
                        dot.edge(str(cntz), str(fa))
                    print(cntz, 'fa=', fa)
                    print('>>> op=',each.state.hisqueue[-2:],'vis=', each.vis)
                    print('>>> rew=', each.reward, 'avg=', each.reward / each.vis)
                    print('>>> rewE=', each.rewardE, 'avgE=', each.rewardE / each.vis)
